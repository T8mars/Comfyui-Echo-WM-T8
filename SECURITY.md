...
- The executable must resolve to an existing file and must not be ComfyUI's
  current Python interpreter.
- The only Python entry point is `inference_wm_causal.py` under the validated
  Echo-WM root.
...
- The node exposes no arbitrary executable, command, module, package-install,
  or Python-code input.
